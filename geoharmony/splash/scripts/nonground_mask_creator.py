...
def get_mask(micasense_path_hdr, dsm_path, kernel_size=30):
    # first step is to make sure they are of the same size
    dsm_cropped = dsm_path.replace(".tif", "_cropped.tif")
    crop_image_to_extent(micasense_path_hdr, dsm_path, dsm_cropped)
    # get the height map
    height_map = get_dsm(dsm_cropped, kernel_size=kernel_size, plot=False)
    # generate a mask for the non ground poitns and dilate it
    # dilated_labels = generate_mask(height_map)

    # using the height map as it is for what we have
    dilated_labels = height_map
    ...
    mask_image_path = dsm_path.replace(".tif", "_mask.tif")
    # write out the labels file based on dsm file
    with rasterio.open(dsm_cropped) as src:
        profile = src.profile
    ...
    print(f'Labels saved as {mask_image_path}')
